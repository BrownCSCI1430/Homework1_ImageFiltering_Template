...
So, given a target image and an original image, suppose we wish to find the kernel that produced the target image. Let $T \in \mathbb{R}^{M\times N}$ be the target image, $I \in \mathbb{R}^{M\times N}$ be our original image, and our kernel $f \in \mathbb{R}^{K \times L}$. As a reminder, convolution is defined as:
$$H[m,n] = (f * I)[m,n] = \sum_{k,l}{f[k,l]I[m-k,n-l]}$$
\end{question}
...
\begin{subquestion}[points=2]
With respect to a kind of filtering that we've learned about, what is the interpretation of your definition of $\frac{\partial{\mathcal{L}}}{\partial{f[k,l]}}$?
\end{subquestion}
...
Convolution with an MSE loss is convex: MSE is a sum of squares (a high-dimensional or hyper-paraboloid, or `quadratic bowl', with a single valley). Since convolution is a linear operator it can only stretch or rotate the paraboloid. Convolution can never create a `second valley' within this paraboloid.
...
Each row of $A$ would be a 2D patch of $I$ that has been flattened, such that $A \in \mathbb{R}^{(M \cdot N) \times (K \cdot L)} $. $x$ would be the flattening of $f$ into a column vector, and $b$ would be the corresponding flattening of $T$ into a vector, such that the matrix-vector product $Ax$ yields the flattened convolution output $b$.
...
As technology advances, evaluating the authenticity of images becomes increasingly difficult. Please read the file \textbf{Grundberg\_NYTimes\_1990.md} transcribed from \href{https://www.nytimes.com/1990/08/12/arts/photography-view-ask-it-no-questions-the-camera-can-lie.html}{this article} by photography critic Andy Grundberg in the \emph{New York Times} from August 1990.
...
The Coalition for Content Provenance and Authenticity (C2PA) has designed a technical specification to attach history to an image. This is now being implemented across the media pipline: e.g., in Canon cameras, Adobe editing products, and on Web browsers. Please watch \href{https://www.youtube.com/watch?v=hA0ZjqakEF8}{this video} for an overview; stop at 4 minutes and 40 seconds.
